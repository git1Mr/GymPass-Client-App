...
async function validateGymAccess(req, res) {
  ...
  if (!userId || !token || !expiresAt || !gymId) {
    return res.status(400).json({
      success: false,
      message: 'Missing required fields: userId, token, expiresAt, gymId.'
    });
  }
  if (typeof expiresAt !== 'number') {
    return res.status(400).json({
      success: false,
      message: 'expiresAt must be a Unix timestamp in milliseconds.'
    });
  }
  ...
  if (req.user.role === 'gym_staff' && req.user.gymId?.toString() !== gymId) {
    return res.status(403).json({
      success: false,
      message: 'You can only validate access for your own gym.'
    });
  }

  try {
    const result = await processGymAccess({
      userId,
      gymId,
      qrToken:     token,
